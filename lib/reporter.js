	...
	structuredStackTrace.toString = function() {
		...
		while (++index < length) {
			...
			line = format(stylize('\t[bold]%s[reset] at %s:%s:%s'),
				_.pad(trace.getMethodName() || trace.getFunctionName() || '<anonymous>', 35, ' '),
...
module.exports.create = function(options) {
	...
	return {
		moduleStart : function(name) {
			current = name;
			console.log(_.pad(format(messages.moduleStart, name) + ' ', 80, '='));
		},
		...
		testDone : function(name, assertions) {
			...
			console.log(_.pad(format(messages.testDone, name) + ' ', 80, '-'));
